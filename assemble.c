...
int make_line(char *string, int type, size_t idx, int *flag,
    ...
    if ( type == comment ){ // comment 인 경우
        ...
        strcpy( line_info[idx].operhand, Pinfo->argu[1]);
        line_info[idx].location = Pinfo->location;
        Pinfo->program_len = line_info[idx-1].location - Pinfo->program_len + 1;
        return 0;
    }
...
void make_objfile(FILE *fp, int start_address, int start_line, line_inform *line_info, object_inform *obj_info) {	//오브젝트 파일 생성 함수
    ...
    fprintf(fp,"H%s  %06X%06X",line_info[idx].symbol, line_info[idx].location, start_address);

    while(1) {	//end가 아닐 때 까지
        if(e_flag == 1)
            break;
        i++;
        fprintf(fp,"\nT%06X%02X",obj_info[i].start, obj_info[i].size);
        while(1) {
            idx++;

            if(get_asmd(line_info[idx].asmd) == end) {
                e_flag = 1;
                break;
            }

            if(get_asmd(line_info[idx].asmd) == resb || get_asmd(line_info[idx].asmd) == resw) { //resb, resw를 처리하는 부분	
                if(line_info[idx].n_flag == 1) { //줄바꿈을 한다
                    while(line_info[++idx].n_flag == 1);
                }
            }
            else if(line_info[idx].obj_code != -1 && line_info[idx].obj_code != -2) {
